...
from packages.core.enums import AgentStatus, MandateStatus, OperationStatus, PrincipalRole, TransactionStatus
...
async def test_hierarchical_budget_spend_propagation_upward(async_client: AsyncClient) -> None:
    ...
    async with TestingSessionLocal() as session:
        principal = Principal(name="Hierarchical Spend Corp", email="spend@mandate.dev", role=PrincipalRole.ADMIN)
        session.add(principal)
        await session.flush()

        parent_agent = Agent(name="Parent Shopping Agent", owner_id=principal.id, agent_type="SHOPPING", api_key_hash="hash_h_p")
        child_agent = Agent(name="Child Procurement Agent", owner_id=principal.id, agent_type="PROCUREMENT", api_key_hash="hash_h_c")
...
@pytest.mark.asyncio
async def test_in_flight_operation_settlement_after_parent_revocation(async_client: AsyncClient) -> None:
    """
    ...
    """
    async with TestingSessionLocal() as session:
        principal = Principal(name="InFlight Corp", email="inflight@mandate.dev", role=PrincipalRole.ADMIN)
        session.add(principal)
        await session.flush()

        parent_agent = Agent(name="InFlight Parent", owner_id=principal.id, api_key_hash="hash_if_p")
    ...
    rev_res = await async_client.post(f"/api/v1/mandates/{parent_id}/revoke", json={"reason": "Compromised admin key"})
    ...
    wh_payload = {
        ...
        "payload": {
            "payment": {"entity": {"id": "pay_inflight_01", "order_id": f"order_mock_{op_id[:8]}", "amount": 500000, "status": "captured"}},
        },
    }
...
async def test_risk_evaluation_agent_structured_signal_flow() -> None:
    ...
    """
    async with TestingSessionLocal() as session:
        principal = Principal(name="Risk Commerce Corp", email="risk@mandate.dev", role=PrincipalRole.ADMIN)
        session.add(principal)
        await session.flush()

        parent_agent = Agent(name="Master Shopping Bot", owner_id=principal.id, agent_type="SHOPPING", api_key_hash="hash_rc_p")
        child_agent = Agent(name="Procurement Sub Bot", owner_id=principal.id, agent_type="PROCUREMENT", api_key_hash="hash_rc_c")
        session.add_all([parent_agent, child_agent])
        await session.flush()
